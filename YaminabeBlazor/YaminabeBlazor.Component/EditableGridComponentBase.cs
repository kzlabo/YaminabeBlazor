...
namespace YaminabeBlazor.Component
{
    ...
    /// </revisionHistory>
    public class EditableGridComponentBase<TItem> : ComponentBase, IFilterComponent, IDisposable
        where TItem : IEditableViewModel, new()
    {
        ...
        /// </summary>
        [Parameter]
        public string EditRowClass { get; set; }

        /// <summary>
        /// 削除済みの行クラスを取得または設定します。
        /// </summary>
        [Parameter]
        public string DeletedRowClass { get; set; }

        /// <summary>
        /// フィルタ条件コンポーネントリストを取得または設定します。
        /// </summary>
        ...
        {
            if (this.DeletedRowClass != null && item.EditState.HasFlag(EditStateOptions.Deleted))
            {
                return this.DeletedRowClass;
            }
            return item.EditMode == EditModeOptions.Edit ? this.EditRowClass : null;
        }
